import { c as consola, a as colors$2, d as defineCommand } from '../shared/gem-cli.19708311.mjs';
...
var dom_serializer_1 = __importDefault$7(lib$3);
var domelementtype_1 = lib$4;
/**
 * @category Stringify
...
var HTMLElement = /** @class */ (function (_super) {
    ...
    Object.defineProperty(HTMLElement.prototype, "structuredText", {
        ...
        get: function () {
            ...
            function dfs(node) {
                if (node.nodeType === type_1$1.default.ELEMENT_NODE) {
                ...
                else if (node.nodeType === type_1$1.default.TEXT_NODE) {
    ...
    HTMLElement.prototype.replaceWith = function () {
        ...
        var content = nodes
            .map(function (node) {
            if (node instanceof node_1$1.default) {
                return [node];
            }
            else if (typeof node == 'string') {
                var r = parse$2(node, _this._parseOptions);
                return r.childNodes.length ? r.childNodes : [new text_1$1.default(node, _this)];
            }
            return [];
        })
            .flat();
    ...
    HTMLElement.prototype.removeWhitespace = function () {
        ...
        this.childNodes.forEach(function (node) {
            ...
            else if (node.nodeType === type_1$1.default.ELEMENT_NODE) {
    ...
    HTMLElement.prototype.insertAdjacentHTML = function (where, html) {
        ...
        var p = parse$2(html, this._parseOptions);
        if (where === 'afterend') {
            ...
            if (is_block_text_element(tagName)) {
                ...
                if (element_should_be_ignore(tagName)) {
                    ...
                    if (text.length > 0 && /\S/.test(text)) {
                        currentParent.appendChild(new text_1$1.default(text, currentParent, createRange(tagEndPos_1, textEndPos)));
                    }
                }
                if (closeIndex === -1) {
...
function writeFile(dirPath, {
  ...
  try {
    ...
    consola.start(colors$2.yellow(`Creating file: ${name}`));
    consola.success(colors$2.green(`Create file: ${name} successfully!`));
  } catch (error_) {
    if (error_.code === "ENOENT") {
      consola.error(colors$2.red(`File does not exist: ${error_.message}`));
    } else if (error_.code === "EISDIR") {
      consola.error(colors$2.red(`Cannot write to a directory: ${error_.message}`));
    } else {
      consola.error(colors$2.red(`Error writing file: ${error_.message}`));
    }
  }
}
...
function makeFile(fileName, fileType) {
  return `${fileName}.${fileType}`;
}
function capitalizeFirstLetter(string) {
  return string.charAt(0).toUpperCase() + string.slice(1);
}
const convertStringToCamelCase = (str, extension) => {
  const words = str.split(" ");
  const camelCaseWords = words.map((word) => {
    return word.charAt(0).toUpperCase() + word.slice(1).toLowerCase();
  });
  const camelCaseString = camelCaseWords.join("");
  const finalString = extension ? camelCaseString + `.${extension}` : camelCaseString;
  return finalString;
};
const convertKebabToTitleCase = (input) => {
  return input.split("-").map((word) => word.charAt(0).toUpperCase() + word.slice(1)).join(" ");
};
const removeSpecialChars = (originalString) => {
...
function AppReactComponent(appConfig) {
...
function AppReactWithAppBlockComponent(appConfig) {
  const { id, label, logoUrl, tag, capitalizedFileName } = appConfig;
  return `import type { AlignProp, BaseProps, ObjectDevices } from '@gem-sdk/core';
import { makeStyleResponsive } from '@gem-sdk/core';
import ThirdPartyPreview from './ThirdPartyPreview';
...
function AppLiquidComponent(config) {
...
function AppLiquidWithAppBlockComponent(config) {
  const { capitalizedFileName } = config;
  return `
    import type { ${capitalizedFileName}Props } from './${capitalizedFileName}';
...
function AppSettingTemplate(config) {
...
const WidgetSettingTemplate = (widgets) => {
  if (!widgets)
    return "";
  const widgetOptionsTemplate = widgets.map((widget) => {
    return `{
...
function AppSettingWithAppBlockTemplate(config) {
...
const AppBlockConfigTemplate = (config) => {
...
const AppBlockSettingTemplate = (config) => {
  const { capitalizedFileName, tag, widgets } = config;
  if (!widgets)
    return "";
  const settingByWidgetTemplate = widgets.map((widget) => {
    return `'${widget}': null,`;
  }).join("\n");
...
let config = {
  id: "",
  logoUrl: "https://ucarecdn.com/e60d4b13-c990-4676-b1cb-50e18ce6911a/-/format/auto/-/preview/3000x3000/-/quality/lighter/",
...
const _initThirdPartyTemplate = async (isUsingAppBlock) => {
  try {
    ...
    if (!sourceURL) {
      consola.error(colors$2.red("No source URL found"));
      rl.close();
      return;
    }
    const url = new URL(sourceURL);
    if (url.hostname !== "apps.shopify.com") {
      consola.error(colors$2.red("Source URL is not from Shopify App Store"));
      rl.close();
      return;
    }
    ...
    if (request.status !== 200) {
      consola.error(colors$2.red("Error fetching data"));
      rl.close();
      return;
    }
    const pageContent = await request.text();
    const document = parse_2(pageContent);
    const adpHero = document.querySelector("#adp-hero");
    if (!adpHero) {
      consola.error(colors$2.red("No adp-hero found"));
      rl.close();
      return;
    }
    ...
    if (isUsingAppBlock) {
      const appName = await question(`Please enter app name: `);
      if (!appName) {
        consola.error(colors$2.red("App name cannot be empty"));
        rl.close();
        return;
      }
      const appId = await question(`Please enter app id: `);
      if (!appId) {
        consola.error(colors$2.red("App id cannot be empty"));
        rl.close();
        return;
      }
...
If there are multiple widgets please separate them by comma (Eg: widget1,widget2):`
      ...
      if (!widgets) {
        consola.error(colors$2.red("Please enter at least one widget"));
        rl.close();
        return;
      }
      config = {
    ...
    const appReactComponent = isUsingAppBlock ? AppReactWithAppBlockComponent(config) : AppReactComponent(config);
    const appLiquidComponent = isUsingAppBlock ? AppLiquidWithAppBlockComponent(config) : AppLiquidComponent(config);
    const appSettingTemplate = isUsingAppBlock ? AppSettingWithAppBlockTemplate(config) : AppSettingTemplate(config);
...
async function fileImported(fileName, fileType) {
  const importers = {
    tsx: importFileReact,
  ...
  if (importer) {
    ...
    consola.error(colors$2.red(`Unsupported file type: ${fileType}`));
  }
}
async function updateAppConfigContent(config2) {
  fs.readFile(paths.thirdPartyAppConfig, "utf8", (err, data) => {
    if (err) {
      consola.error("Error reading the file:", err);
      return;
    }
    const newConfigTemplate = AppBlockConfigTemplate(config2);
    const updatedContent = data.trimEnd() + "\n\n" + newConfigTemplate;
    fs.writeFile(paths.thirdPartyAppConfig, updatedContent, "utf8", (err2) => {
      if (err2) {
        consola.error("Error writing to the file:", err2);
      } else {
        consola.success(colors$2.green("New block config added successfully!"));
      }
    });
  });
}
async function updateAppSettingContent(config2) {
  fs.readFile(paths.thirdPartyAppSetting, "utf8", (err, data) => {
    if (err) {
      consola.error("Error reading the file:", err);
      return;
    }
    const newSettingTemplate = AppBlockSettingTemplate(config2);
    const exportStatement = "export const composeSettingsByWidgetType: Record<string, any> = {";
    ...
    if (insertPosition === -1) {
      ...
      data = data.replace(
        exportStatement,
        `${exportStatement}
  ...${config2.capitalizedFileName},`
      );
      fs.writeFileSync(paths.thirdPartyAppSetting, data);
      consola.success(colors$2.green("New block setting added successfully."));
    }
  });
}
async function updateAppConstantContent(config2) {
  fs.readFile(paths.thirdPartyConstant, "utf8", (err, data) => {
    if (err) {
      consola.error("Error reading the file:", err);
      return;
    }
    const { capitalizedFileName } = config2;
...
${closingBrace}`;
    ...
    consola.success(colors$2.green("File constant updated successfully."));
  });
}
function updateFileContent(args) {
  const { filePath, newImport, newExport, newExportPropsType } = args;
  fs.readFile(filePath, "utf8", (err, data) => {
    if (err) {
      consola.error(colors$2.red(`Error reading file: ${err}`));
      return;
    }
    ...
    const updatedContent = data.replace(importRegex, `$&${newImport}`).replace(
      exportRegex,
      (match, p1) => `export {
  ${newExport}
${p1}
...
${p1}
    ...
    consola.success(colors$2.green(`File updated successfully`));
  });
}
function importFileReact(fileName) {
...
function importFileNext(fileName) {
...
function importFileBuilder(fileName) {
...
function importFileLiquid(fileName) {
...
function importSettingFile(fileName) {
...
const colors$1 = /*@__PURE__*/getDefaultExportFromCjs(yoctocolorsCjs);
...
const require$$0 = /*@__PURE__*/getAugmentedNamespace(spinners$3);
...
const spinners$1 = /*@__PURE__*/getDefaultExportFromCjs(cliSpinners);
...
const cliWidth$1 = /*@__PURE__*/getDefaultExportFromCjs(cliWidth_1);
...
const stripAnsi$3 = /*@__PURE__*/getDefaultExportFromCjs(stripAnsi$2);
...
const wrapAnsi$1 = /*@__PURE__*/getDefaultExportFromCjs(wrapAnsi_1);
...
const MuteStream$1 = /*@__PURE__*/getDefaultExportFromCjs(lib);
...
const selectTheme = {
    ...
    style: { disabled: (text) => colors$1.dim(`- ${text}`) },
    helpMode: 'auto',
};
